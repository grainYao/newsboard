...
export default function AiAssistant() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 text-white relative overflow-hidden">
      ...
      {/* ── Header ── */}
      <div className="relative z-10 px-5 pt-5 pb-4 border-b border-slate-800/60">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-white">RSS 发现助手</h2>
            <p className="text-[10px] text-slate-500">用自然语言找到想要的订阅源</p>
          </div>
        </div>
        <div className="bg-slate-900/70 backdrop-blur-sm rounded-xl p-3 border border-slate-700/50">
          <SearchConfigForm />
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-5 py-4 space-y-6 scrollbar-thin">
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full space-y-4 animate-fade-in">
            ...
            <div className="text-center">
              <p className="text-sm text-slate-300 font-medium">探索 RSS 宇宙</p>
              <p className="text-xs text-slate-600 mt-1">试试：科技新闻 · 编程教程 · AI 前沿 · 动漫资讯</p>
            </div>
            <div className="flex gap-2 flex-wrap justify-center max-w-xs">
              {['科技新闻', '编程博客', 'AI前沿', '游戏电竞'].map((tag) => (
                <button
                  ...
                  className="px-3 py-1 text-xs rounded-full bg-slate-800/80 border border-slate-700 text-slate-400 hover:border-cyan-400/50 hover:text-cyan-400 transition-colors cursor-text"
                >
                  {tag}
                </button>
              ))}
        ...
        {messages.map((msg, msgIdx) => (
          <div
            key={msg.id}
            className={`space-y-3 animate-message-in ${msgIdx === messages.length - 1 ? '' : ''}`}
            style={{ animationDelay: '0ms' }}
          >
            {msg.role === 'user' ? (
              <div className="flex justify-end animate-message-in" style={{ animationDelay: '0ms' }}>
              ...
              <div className="space-y-3 animate-message-in" style={{ animationDelay: '0ms' }}>
                {/* AI avatar + text */}
                ...
                {msg.sources && msg.sources.length > 0 && (
                  <div className="pl-0 space-y-2">
                    <p className="text-[10px] uppercase tracking-widest text-slate-600 font-medium px-1">
                    ...
                    <div className="grid gap-2">
                      {msg.sources.map((s, i) => {
                        ...
                        return (
                          <div
                            key={s.url}
                            className="group relative bg-slate-900/80 backdrop-blur-md rounded-xl p-4 border border-slate-700/60 hover:border-cyan-400/50 transition-all duration-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] animate-source-card"
                          ...
                          >
                            ...
                            <div className="flex items-start gap-3 relative">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-white truncate group-hover:text-cyan-200 transition-colors">{s.name}</p>
                                <p className="text-[11px] text-slate-500 font-mono mt-0.5 truncate">{s.url}</p>
                                {s.description && (
                                  <p className="text-xs text-slate-400 mt-1.5 leading-relaxed line-clamp-2">{s.description}</p>
                                )}
                                {s.suggested_category && (
                                  <div className="mt-2 flex items-center gap-1.5">
                                    <span className="px-2 py-0.5 text-[10px] rounded-full bg-teal-500/15 border border-teal-500/30 text-teal-300 font-medium">
                                      {s.suggested_category}
                                    </span>
                                  </div>
                                )}
                              ...
                              <button
                                ...
                                className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 ${
                                  isAdded
                                    ...
                                    : 'bg-slate-800 border border-slate-600 text-slate-400 hover:bg-cyan-500 hover:border-cyan-400 hover:text-white group-hover:shadow-[0_0_12px_rgba(34,211,238,0.4)]'
                                }`}
                              >
                                {isAdding ? (
      ...
      <div className="relative z-10 px-5 pb-5">
        <div className="relative">
          ...
          <div className="relative flex items-center gap-2 bg-slate-900/90 backdrop-blur-xl rounded-2xl border border-slate-700/80 px-4 py-2 focus-within:border-cyan-500/60 transition-colors">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSend()}
              placeholder="描述你想要的 RSS 源..."
              className="flex-1 bg-transparent text-sm text-white placeholder-slate-600 focus:outline-none py-1"
              disabled={loading}
            />
            <button
              onClick={handleSend}
